pub use self::stardust::possible_levels;
...
mod stardust {
    ...
    /// level. If `cost_to_powerup` is not a valid value (i.e., there is no Pokemon level that
    /// requires that amount of dust) returns `None`.
    ///
    /// Example:
    ///
    /// ```
    /// use pokemon_iv::stardust::possible_levels;
    ///
